...
export default function SupervisorDashboard() {
  ...
  const usersChartData = {
    labels: ['Administrador', 'Asignador', 'Calificador', 'Responsable', 'Supervisor'],
    datasets: [
      {
        label: 'Número de Usuarios',
        data: [usersByRole.admin, usersByRole.asignador, usersByRole.calificador, usersByRole.responsable, usersByRole.supervisor],
        backgroundColor: [
          'rgba(239, 68, 68, 0.8)',
          'rgba(245, 158, 11, 0.8)',
          'rgba(59, 130, 246, 0.8)',
  ...
  const generatePDFReport = async () => {
    ...
    doc.text('REPORTE DE SUPERVISOR', 120, currentY + 55);
    ...
    doc.autoTable({
      ...
      body: [
        ...
        ['Supervisores', usersByRole.supervisor, `${((usersByRole.supervisor / users.length) * 100).toFixed(1)}%`],
    ...
    doc.save(`Reporte_UNMSM_Supervisor_${format(new Date(), 'yyyy-MM-dd_HHmm')}.pdf`);
  };
  ...
  const generateExcelReport = async () => {
    ...
    const resumenData = [
      ...
      ['REPORTE DE SUPERVISOR - BALANCED SCORECARD'],
      [''],
    ...
    XLSX.writeFile(wb, `Reporte_UNMSM_Supervisor_${format(new Date(), 'yyyy-MM-dd_HHmm')}.xlsx`);
  };
