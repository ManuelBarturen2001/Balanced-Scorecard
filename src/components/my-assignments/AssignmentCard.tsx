...
export function AssignmentCard({ assignedIndicator, onViewDetails, onFileUpload }: AssignmentCardProps) {
  ...
  //@ts-ignore
  if (overallStatus === 'Pending' && assignedIndicator.assignedVerificationMethods.some(vm => vm.dueDate && isPast(new Date(vm.dueDate?.seconds * 1000)) && vm.status === 'Pending')) {
